...
int main()
{
    ...
    // to store updates in log files,everytime there is an update by using switch case

    int choice;
    while (1)
    {
        printf("\nChoose 1 if there are any updates....\n");
        printf("choose 2 if there are no updates...\n");
        scanf("%d", &choice);
        switch (choice)
        {
        case 1:
            create_copy(); //calling function to create log files and copy the data in log files to master files
            break;
        case 2:
            exit(1);
        default:
...
void create_copy()
{
    ...
    sprintf(filename, "3203_3231_update_%s_%s", date, time);
